...
1;
switch handles.Geometry_popupmenu.Value
    case 1
% Hexagonal lattic
...
%         PartialCharge=protein(whichIs).tail.(handles.PhosState).PartialCharge;
        ...
        if ~handles.New_P_Conf_check.Value
...
TailFirstOsmoticTerm_tot=0;         % Following correspondence with Zhulina
switch ModelName
%     case h.Children(1)
%         model='Some_spring'
    case 'Flory'          % Flory
...
%         P_mono_thermal=P_mono_prefactor*SetUnits(xi_thermal.^-3);
        ...
        P_mono=P_mono_real;
        handles.Corr_text.String=['at 40nm: ' num2str(xi(D==40))];
    case 'Neutral Polymer Scaling (cylinders)'
        % Macromolecules 1980,13, 1280-1289 Pincus. 
        model='NeutScal'
        tau=1; % 1 for thermal solvent, zero for theta solvent. In Rubinstein's book, this would be \frac{\nu}{b^3}
        Prefactor=0.60;
        n=1/kuhn; %n=a/b in paper
        xi=Prefactor*(a*(V_aa*c_p_tot(D)).^(-3/4)*n^(-1/4)*tau^(-1/4)); % this is eq. 15. They made a mistake there: it should have been (a^3*\rho), not (a*\rho^3). Just consider the units (or see the scaling in Rubinstein page 188) and you will agree. 
        P_mono=P_mono_prefactor*SetUnits(xi.^-3);
        handles.Corr_text.String=['at 40nm: ' num2str(xi(D==40))];
    case 'Scaling cylinder'
        % Polymer Science U.S.S.R. Vol.30, No. 4, pp. 772-779. 1988, and mostly my own derivation in "comments on scaling".  
        model='CylScal'
        Prefactor=0.02;
        R=handles.CylRadius;
        Num_of_tails_in_Cyl=32;
        s=Num_of_tails_in_Cyl/(45*2*pi*R);
        P_mono_no_units=1/45*a^(-5/3)*(1/R/s)^(-5/6)...
            *(D).^(-5/6)./(D.^(5/6)-R.^(5/6))*...
            Num_of_tails_in_Cyl*dot(ProteinRatios,Length);
        P_mono=Prefactor*P_mono_prefactor*SetUnits(P_mono_no_units);
end
...
1;
if handles.New_P_Conf_check.Value
    NumOfProteinTypes=length(ProteinList);
%     NiNj_mat=repmat(Length,[NumOfProteinTypes 1])+repmat(Length,[NumOfProteinTypes 1])';
    NiNj_mat=bsxfun(@plus,Length',Length);
    MiMj_mat=bsxfun(@(A,B)A.*B,ProteinRatios',ProteinRatios);
    Weights=MiMj_mat./NiNj_mat;
    Weights(isnan(Weights))=0;
    
    sum_P_conf=-4*SetUnits(Num_of_tails_in_Vol*...
                3/2*H(D)/a^2./V_diff(D)*0.5*sum(sum(Weights)))/kuhn; % 0.5 is important because we sum i>j
