...
%         P_el_Rub=((c_p_tot(D)).^2)./(4*A_Rub^2*C_S+A_Rub*c_p_tot(D)); % 1995 paper term for ionic osmotic pressure? If so, remove from here. 
        xi_zero=b*(u*f_star^2)^(-1/6)*(c_p_tot(D)*V_aa).^(-1/2);    % un-numbered equation right after equation 3.17
...
%% Export model fit and A parameters
...
if ExportInAngstorm
    Matlab_x=10*x;
    Matlab_x_exp=10*x_exp;
end

if handles.IsX_axis_water_volume
    filename=[NewFolder 'Water_volume_' filename ext];
    ExportMat=[Matlab_x; y]';
    NaN_vec=nan(numel(Matlab_x)-numel(Matlab_x_exp),1);  % also include the experimental data!
    ExportMat=[ExportMat [Matlab_x_exp; NaN_vec] [y_exp; NaN_vec]];
else if handles.IsX_SlopePlot
        ...
        ExportMat=[Matlab_x; y; P_ion; P_mono; sum_P_conf]';
    end
end
